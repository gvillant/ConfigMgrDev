        ...
        foreach ($a in $applicationXML.DeploymentTypes)
            {
                ...
                $DTNameFull = $a.Title
                $DTName = $DTNameFull.substring(5)
                $DTName =  $DTName.subString(0, [System.Math]::Min(50, $DTName.Length))

                
                write-host "Processing $DTNameFull" -ForegroundColor Green

                if (Get-CMPackage -Name "$($DTName)")
                        {
                    ...
                    New-CMPackage -Name "$($DTName)" -Path $ContentPath 
                    New-CMProgram -PackageName "$($DTName)" -StandardProgramName $DTName -RunMode RunWithAdministrativeRights -UserInteraction $false -RunType Hidden -ProgramRunType WhetherOrNotUserIsLoggedOn -CommandLine $InstallCommandLine 

                    $Package = Get-CMPackage -Name "$($DTName)"

...
SET @P_OStype = 'Workstation';

DECLARE @P_LastActivity DATETIME = DATEADD(d, -14 ,GETDATE());


SELECT DISTINCT 
SYS.Netbios_Name0 as Name,@P_LastActivity,
SYS.Obsolete0,
CASE SYS.Obsolete0
	WHEN '0' THEN  'Managed'
	ELSE 'Not Managed'
...
CASE 
	WHEN SYS.Last_Logon_Timestamp0 > @P_LastActivity THEN 'Active'
	ELSE 'Inactive'
	END AS 'Activity'
...
WHERE (LDISK.DeviceID0 IS NULL or LDISK.DeviceID0 = 'C:') -- and SYS.Operating_System_Name_and0 like ('%' + @P_OStype + '%')
--and SYS.Obsolete0 = 1 
...
,LDISK.Description0 , LDISK.DeviceID0, LDISK.VolumeName0, LDISK.FileSystem0,LDISK.Size0, LDISK.FreeSpace0,(LDISK.Size0 - LDISK.FreeSpace0), SYS.AD_Site_Name0,SYS.Last_Logon_Timestamp0

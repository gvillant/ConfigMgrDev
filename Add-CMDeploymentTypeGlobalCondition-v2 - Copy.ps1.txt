...
#>
<#
$ApplicationName = "MADEXCEPT" #$Appdt.LocalizedDisplayName
$DeploymentTypeName = "MADSHI_MADEXCEPT_4.0.10_R01_7X64C_FR" #$dt.Title
$dkserver = "FRPSCM01"
$sitecode = "STG"
$GlobalCondition = "os-wks-x64"
#$GlobalCondition = "PrimaryDevice"
$Operator = "IsEquals"
$Value = "True"
#>
Param (
	...
	[parameter(Mandatory = $True)][ValidateSet("OperatingSystem", "TotalPhysicalMemory", "NumberOfProcessors", "OSLanguage", "CPUSpeed", "MachineOU", "PrimaryDevice", "FreeDiskSpace", "SCCMSite", "ADSite", "Device_OwnershipDesktop","OS-WKS-x64", IgnoreCase = $true)] [string]$GlobalCondition,
...
try
{
	$CurrentFolder = (Get-Item (Get-Location).Path).Parent.FullName
	if ($CurrentFolder -eq '') { $CurrentFolder = (Get-Location).Path }
	...
	switch ($GlobalCondition.Tolower())
	{
		...
		"os-wks-x64" {
			$GlobalConditionName = "GlobalExpression_ef5d492b-d101-4c90-b9d4-40be257e1003"
			$ExpressionBase = new-object "Microsoft.ConfigurationManagement.DesiredConfigurationManagement.CustomCollection``1[[Microsoft.SystemsManagementServer.DesiredConfigurationManagement.Expressions.ExpressionBase]]"
			$ConfigurationItemSettingSourceType = [Microsoft.ConfigurationManagement.DesiredConfigurationManagement.ConfigurationItemSettingSourceType]::CIM
			$pdSettingDataType = [Microsoft.SystemsManagementServer.DesiredConfigurationManagement.Expressions.DataType]::Boolean
			$pdDataType = [Microsoft.SystemsManagementServer.DesiredConfigurationManagement.Expressions.DataType]::Boolean
	...
	switch ($GlobalCondition.Tolower())
	{
		...
		{ ($_ -eq "os-wks-x64") } {
			#$ExpressionBase.Add((new-object Microsoft.SystemsManagementServer.DesiredConfigurationManagement.Expressions.GlobalSettingReference -ArgumentList ("ScopeId_CFD257E3-A5C7-4E0B-9714-A19B7B7C0B10", "$GlobalCondition", $pdSettingDataType, "$GlobalConditionName", $ConfigurationItemSettingSourceType)))
            $ExpressionBase.Add((new-object Microsoft.SystemsManagementServer.DesiredConfigurationManagement.Expressions.GlobalExpression -ArgumentList ("ScopeId_CFD257E3-A5C7-4E0B-9714-A19B7B7C0B10", "GlobalExpression_ef5d492b-d101-4c90-b9d4-40be257e1003")))
   ...
   #$newRule = new-object Microsoft.SystemsManagementServer.DesiredConfigurationManagement.Rules.Rule -ArgumentList @("$($GlobalCondition)Rule_$([Guid]::NewGuid().ToString())", [Microsoft.SystemsManagementServer.DesiredConfigurationManagement.Rules.NoncomplianceSeverity]::None, $Annotation, $expression)
    ...
    write-host -ForegroundColor Cyan "Fix application = $ApplicationName - $DeploymentTypeName"
	...
	foreach ($dtype in $AppXML.deploymenttypes)
	{
		if ($dtype.Title.ToLower() -ne $DeploymentTypeName.tolower()) { $i++ }
		else { break }
	}
    #ajoute le nouveau Requirement
    write-host -ForegroundColor Yellow "Add RQ RuleID = $($Newrule.RuleId) - $($Newrule.Name)"
	$AppXML.DeploymentTypes[$i].Requirements.Add($newrule)
	
    #trouve le requirement de type OS
    [array]$rqsToDelete = @()
    $rqtodelete = $null
    $j = 0
    
    foreach ($rq in $appxml.DeploymentTypes[$i].Requirements)
    {
        write-host "Find $j - $($rq.Name) - $($rq.expression.GetType().name)" 
        if ($rq.expression.GetType().name -eq 'OperatingSystemExpression') 
            {
            $rqsToDelete += $j
            }
        $j++
    ...
    # Supprime le(s) requirement(s) de type OS
    foreach($rqToDelete in $rqsToDelete) {
        write-host -ForegroundColor Green "Delete RQ index = $rqToDelete"
        $AppXML.DeploymentTypes[$i].Requirements.RemoveAt($rqToDelete)
        }
...
{
	cd "$($CurrentFolder[0]):"
	$module = get-module -Name ConfigurationManager | where-object { $_.ModuleType -eq "Manifest" }
	if ($module -ne $null)
	{
